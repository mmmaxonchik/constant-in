...
ConductorSetup::~ConductorSetup() {
    // egalito is owned by us (created in parseEgalito).
    // elf is a borrowed pointer from the first module's ElfSpace — do NOT delete it.
    delete egalito;
    egalito = nullptr;

    // Deleting conductor cascades through Program → Module → ElfSpace → FunctionList.
    // Clear the global before delete to avoid dangling pointer.
    if(::egalito_conductor == conductor) ::egalito_conductor = nullptr;
    delete conductor;
    conductor = nullptr;
}
