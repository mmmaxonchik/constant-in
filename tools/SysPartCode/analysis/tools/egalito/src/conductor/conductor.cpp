...
Conductor::~Conductor() {
    // LibraryList is not a ChunkList child of Program, so it must be deleted
    // explicitly. ChunkListImpl will delete the Library objects inside it.
    if(program) {
        delete program->getLibraryList();
        program->setLibraryList(nullptr);
    }
    delete program;
}
...
void Conductor::parseLibraries() {
    ...
    for(size_t i = 0; i < iterable->getCount(); i ++) {
        ...
        const char *path = library->getResolvedPathCStr();

        if(!path || path[0] == '\0') {
            std::string name = library->getName();
            std::fprintf(stderr,
                         "[syspart_new] WARNING: skipping library with empty resolved path (name=%s)\n",
                         name.c_str());
            continue;
        }

        ElfMap *elf = new ElfMap(path);
        parse(elf, library);
    }
}
