...
/* Run a full reachability analysis starting from start_func without NSS
   resolution (safe for container-image binaries). Returns all syscalls
   reachable from the entry-point function. */
set<int> Syspart::analyzeSyscalls(bool icanalysisFlag)
{
    ...
    SysNode *node = getSysNode(start_func);
    if (!node) return {};
    return getSyscalls(node);
}

Syspart::SysNode* Syspart::prepareForPropagation(bool icanalysisFlag)
{
    if (!start_func) return nullptr;
    ...
    // Release the DataFlow use-def cache accumulated during generateIndirectEdges.
    // With ic_analysis=true and many AT-functions, this cache can reach 10-20 GB.
    // It was allocated last (highest heap addresses), so freeing it now lets
    // malloc_trim shrink the brk before findDirectSyscalls adds SysNode objects.
    ip_callgraph.clearDataFlow();
    malloc_trim(0);

    findDirectSyscalls();
    return getSysNode(start_func);
}

std::set<int> Syspart::finalizeSyscalls(SysNode* startNode)
{
    if (!startNode) return {};
    findDerivedSyscalls(nullptr);
    return getSyscalls(startNode);
}
	...
	for(auto i : ip_callgraph.nodeMap)
	{
        ...
        for(auto kv : list)
        {
            ...
            for(auto value : syscallValues)
            {
                ...
                who_invokes_syscalls[value].push_back(s);
            }
        }
        (s->syscall_info)[s] = bs;
            ...
            if(sys_node_iter != syscall_mapping.end())
            {
                ...
                for(auto pp : par)
                {
                    ...
                    if(par_iter == syscall_mapping.end())
                        continue;
                    SysNode *parnode = par_iter->second;
...
{
    // Single-pass topological propagation — replaces per-syscall DFS.
    //
    // Old algorithm: O(N_syscalls × N_direct_callers × N_nodes)
    // New algorithm: O(N_nodes + N_edges) for topo sort +
    //                O((N_nodes + N_edges) × SYSCALL_BITS/64) for bitset propagation +
    //                O(cycle_length) fixed-point iterations
    //
    // Step 1: build a bitset per call-graph node initialised from direct syscalls.
    // Step 2: DFS to produce a post-order (children before parents), handling
    //         cycles by skipping already-visited nodes.
    // Step 3: one propagation pass in post-order (each node ORs its bits into
    //         every parent).
    // Step 4: fixed-point loop to close cycles (converges in O(max_cycle_len)
    //         iterations — typically 1-2 for real call graphs).
    // Step 5: write derived_syscalls and syscall_info back from the bitsets.

    const size_t N = syscall_mapping.size();

    // --- Step 1 ---
    unordered_map<IPCallGraphNode*, bitset<SYSCALL_BITS>> bits;
    bits.reserve(N);
    for(auto& [ipnode, sysnode] : syscall_mapping) {
        auto& b = bits[ipnode];   // default-constructed to all-zero
        for(int sysno : sysnode->direct_syscalls)
            if(sysno >= 0 && static_cast<size_t>(sysno) < SYSCALL_BITS)
                b.set(static_cast<size_t>(sysno));
    }

    // --- Step 2: iterative DFS → post_order ---
    vector<IPCallGraphNode*> post_order;
    post_order.reserve(N);
    unordered_set<IPCallGraphNode*> visited;
    visited.reserve(N);

    // Stack holds (node, is_returning).  When is_returning==true we append to
    // post_order; when false we mark visited, push the returning marker, then
    // push unvisited children.
    stack<pair<IPCallGraphNode*, bool>> stk;

    for(auto& [ipnode, _] : syscall_mapping) {
        if(visited.count(ipnode)) continue;
        stk.push({ipnode, false});
        while(!stk.empty()) {
            auto [n, returning] = stk.top();
            stk.pop();
            if(returning) {
                post_order.push_back(n);
                continue;
            }
            if(!visited.insert(n).second) continue;  // already visited
            stk.push({n, true});   // returning marker
            for(auto& [addr, child_set] : n->getDirectChildrenRef())
                for(auto* child : child_set)
                    if(bits.count(child) && !visited.count(child))
                        stk.push({child, false});
            for(auto& [addr, child_set] : n->getIndirectChildrenRef())
                for(auto* child : child_set)
                    if(bits.count(child) && !visited.count(child))
                        stk.push({child, false});
        }
    ...
    // --- Step 3: single upward propagation pass ---
    for(auto* node : post_order) {
        const auto& self = bits[node];
        if(self.none()) continue;
        for(auto& [addr, par] : node->getParentMap()) {
            auto it = bits.find(par);
            if(it != bits.end()) it->second |= self;
        }
    }

    // --- Step 4: fixed-point for cycles ---
    bool changed = true;
    while(changed) {
        changed = false;
        for(auto* node : post_order) {
    ...
    // --- Step 5: write back ---
    for(auto& [ipnode, sysnode] : syscall_mapping) {
        const auto& total = bits[ipnode];
        if(total.none()) continue;
        for(size_t i = 0; i < SYSCALL_BITS; i++) {
            if(total.test(i) && !sysnode->direct_syscalls.count(static_cast<int>(i)))
                sysnode->derived_syscalls.insert(static_cast<int>(i));
        }
        // syscall_info[child] = all syscalls reachable through that direct child
        for(auto& [addr, child_set] : ipnode->getDirectChildrenRef()) {
            for(auto* child : child_set) {
                auto ci = syscall_mapping.find(child);
                if(ci == syscall_mapping.end()) continue;
                const auto& cb = bits[child];
                if(cb.any()) sysnode->syscall_info[ci->second] |= cb;
            }
        }
        for(auto& [addr, child_set] : ipnode->getIndirectChildrenRef()) {
            for(auto* child : child_set) {
                auto ci = syscall_mapping.find(child);
                if(ci == syscall_mapping.end()) continue;
                const auto& cb = bits[child];
...
bool Syspart::findFunctionsReachable(address_t addr, Function* func, string end)
{
    ...
    auto cfg_owner = std::unique_ptr<ControlFlowGraph>(new ControlFlowGraph(func));
    auto cfg = cfg_owner.get();
...
set<int> Syspart::findSyscallsAccessible(address_t addr, Function* f)
{
    ...
    auto cfg_owner = std::unique_ptr<ControlFlowGraph>(new ControlFlowGraph(f));
    auto cfg = cfg_owner.get();
    stack<Block*> block_stack;
...
{
    auto graph   = std::unique_ptr<ControlFlowGraph>(new ControlFlowGraph(f));
    auto config  = std::unique_ptr<UDConfiguration>(new UDConfiguration(graph.get()));
    auto working = std::unique_ptr<UDRegMemWorkingSet>(new UDRegMemWorkingSet(f, graph.get()));
    auto usedef  = std::unique_ptr<UseDef>(new UseDef(config.get(), working.get()));
    SccOrder order(graph.get());
    order.genFull(0);
...
void Syspart::findReachableCode(bool direct, bool icanalysisFlag, bool typearmorFlag, address_t addr, string func_name)
{
   ...
   if(start_bl == NULL)
    ...
    set<Block*> visited;
    noreturn_done.clear();
    auto graph_owner = std::unique_ptr<ControlFlowGraph>(new ControlFlowGraph(func));
    int ret = isNonReturn(graph_owner.get(), start_bl, visited);
    if(ret == 1)
    {
        cout<<"\n Is non-returning \n";
    }
