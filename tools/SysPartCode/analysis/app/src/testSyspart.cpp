...
{ 
    ...
    { 
        ...
        case 'r' : sysroot_path = arg;
                   break;
        case 'L' : library_path = arg;
                   break;
    }
    return 0;
}
...
int main(int argc, char *argv[])
{
    ...
    { {0, 'p', "PROG", 0, "The binary program to be analyzed"},
      ...
      {0, 'r', "SYSROOT", 0, "Sysroot prefix for resolving shared libraries. Use this to analyze binaries from a mounted container image, e.g. '-r /mnt/bundle'"},
      {0, 'L', "LIBPATH", 0, "Colon-separated list of additional directories to search for shared libraries, e.g. '-L /mnt/bundle/lib:/mnt/bundle/usr/lib'"},
    ...
    if (!sysroot_path.empty()) {
        setenv("EGALITO_SYSROOT", sysroot_path.c_str(), 1);
    }
    if (!library_path.empty()) {
        setenv("EGALITO_LIBRARY_PATH", library_path.c_str(), 1);
    }
