...
class Syspart
{
	public :
		...
		// x86_64 native: max ~435 (clone3); x32 ABI: max 547 (pwritev2).
		// 600 covers all known Linux syscalls with a small safety margin,
		// and is 3.4× smaller than 2048 — significant for per-edge bitsets.
		static constexpr size_t SYSCALL_BITS = 600;
	...
	private :
		...
		unordered_map<IPCallGraphNode*, SysNode*> syscall_mapping;	//IPCallGraphNode to SysNode mapping (IPCallGraph)
	...
	public :
		~Syspart()
		{
			for(auto& kv : syscall_mapping)
				delete kv.second;
			syscall_mapping.clear();
		}
		...
		// Library API: run full analysis from start_func, return all syscalls
		// reachable from the entry point. Does not resolve NSS.
		std::set<int> analyzeSyscalls(bool icanalysisFlag = false);

		// Two-phase API that allows the caller to free the egalito IR between
		// the phases to reduce peak memory usage.
		//
		// Phase 1 (egalito IR must be alive): builds the call graph, finds
		// direct syscalls, and captures the start node.  Returns nullptr if
		// the start function has no call-graph entry.
		SysNode* prepareForPropagation(bool icanalysisFlag = false);
		// Phase 2 (egalito IR may be freed): propagates syscalls through the
		// call graph and returns the full reachable set for the start function.
		std::set<int> finalizeSyscalls(SysNode* startNode);
