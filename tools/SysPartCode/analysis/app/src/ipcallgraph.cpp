...
void IPCallGraph::addEdge(address_t addr, Function* start, Function* end, bool isDirect)
{
	...
	if(start_iter == nodeMap.end())
	{
		...
		if(icanalysisFlag) df.getWorkingSet(start);
	...
	if(end_iter == nodeMap.end())
	{
		...
		if(icanalysisFlag) df.getWorkingSet(end);
...
void IPCallGraph::addATFunction(address_t addr, Function* start, Function* ATfunc)
{
	...
	if(ATfunc_iter == nodeMap.end())
	{
		...
		if(icanalysisFlag) df.getWorkingSet(ATfunc);
...
void IPCallGraph::addIndirectSource(address_t addr, Function* func)
{
	...
	if(f_iter == nodeMap.end())
	{
		...
		if(icanalysisFlag) df.getWorkingSet(func);
...
void IPCallGraph::generateIndirectEdges(IPCallGraphNode* n)
{
	...
	{
		// Without IC analysis (icanalysisFlag=false) we have no type information
		// to filter indirect call targets, so every AT function is conservatively
		// treated as a possible target.  Library dispatch tables (e.g. libcrypto's
		// hundreds of EVP algorithm functions) make globalATList huge and create an
		// O(nodes_with_icalls × |globalATList|) connection explosion that OOMs.
		// Restricting to ROLE_MAIN functions keeps the set small while still
		// capturing all address-taken functions defined in the binary under analysis.
		if(!icanalysisFlag)
		{
			auto mod = static_cast<Module*>(f->getParent()->getParent());
			if(mod->getLibrary()->getRole() != Library::ROLE_MAIN)
				continue;
		}
