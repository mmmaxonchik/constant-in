...
class IPCallGraphNode
{
	...
	public :
		...
		// Zero-copy accessor — use this in hot loops instead of getParent()
		const map<address_t, IPCallGraphNode*>& getParentMap() const
		{
			return parent;
		}
		...
		const map<address_t, set<IPCallGraphNode*>>& getDirectChildrenRef() const {
			return direct_children;
		}

		const map<address_t, set<IPCallGraphNode*>>& getIndirectChildrenRef() const {
			return indirect_children;
		}
...
{	
	...
	void addNssEdges();
	// Release the DataFlow use-def cache built during generateIndirectEdges.
	// Safe to call once generate() has returned; the resolved edges are stored
	// in the node maps and the DataFlow objects are no longer referenced.
	// Frees the top-of-heap DataFlow memory so malloc_trim can release it.
	void clearDataFlow() { df = DataFlow{}; }
